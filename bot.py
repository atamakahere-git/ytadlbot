...
from const import (DBHANDLER, HEROKU_APP_NAME, LOGGER, OPEN_CHANNEL_USERNAME,
                   POLLING, PORT, TOKEN)
from databasehandler import add_to_db, check_in_db
from helper import *
...
def start(update: Update, context: CallbackContext) -> None:
    ...
    update.message.reply_text(
        f"Hello {update.effective_user.first_name}, I'm youtube audio downloader bot! "
...
def help_bot(update: Update, context: CallbackContext) -> None:
    ...
    log(update, LOGGER)
    update.message.reply_text(
        f"Use telegram inline bot @vid to search youtube video and send me the link\n"
...
def extract_url_download(update: Update, context: CallbackContext) -> None:
    ...
    yt_urls_msg = update.message.reply_text(
        pretty_url_string(yt_urls), disable_web_page_preview=True
    )
    if len(yt_urls) > 0:
        for url in yt_urls:
            if "list=" in url:
        ...
        context.bot.delete_message(
            message_id=yt_urls_msg.message_id, chat_id=yt_urls_msg.chat_id
        )


def download_playlist_url(
    update: Update, context: CallbackContext, pl_link: str
) -> None:
...
def main() -> None:
    ...
    updater.dispatcher.add_handler(
        CommandHandler("help", help_bot, run_async=True))
    updater.dispatcher.add_handler(
        CommandHandler("donate", donate, run_async=True))
    # Message handler
    updater.dispatcher.add_handler(
        MessageHandler(
            Filters.text & ~Filters.command, extract_url_download, run_async=True
        )
    )
